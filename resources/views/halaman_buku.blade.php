...
<body class="bg-slate-100 font-sans min-h-screen flex flex-col items-center justify-center p-6">
    ...
    <div class="bg-white p-8 rounded-2xl shadow-xl w-full max-w-2xl border border-slate-200">
        ...
        <div class="text-center mb-8">
            ...
            @endif
             ...
             @if(session('error'))
                <div class="mt-4 p-3 bg-red-100 text-green-700 rounded-xl text-sm font-bold border border-green-200 text-center animate-bounce">
                    🎉 {{ session('error') }}
                </div>
            @endif
        </div>
    ...
    <div class="p-4 rounded-xl flex justify-between items-center border 
        ...
        @if($buku->status == 'Tersedia')
            <form method="POST" action="/pinjam-buku/{{$buku->judul}}">
                @csrf
        ...
        @else
            <button disabled class="bg-gray-400 text-white px-4 py-2.5 rounded-xl text-xs font-bold cursor-not-allowed">
                Sedang Dipinjam
            </button>
        @endif
    </div>
